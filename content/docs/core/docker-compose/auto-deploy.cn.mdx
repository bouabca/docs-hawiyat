---
title: 自动部署
description: 如何使用 Hawiyat 自动部署您的 docker-compose 应用
---

自动部署您的 docker-compose 应用到 Hawiyat 可以通过两种主要方法实现：使用 Webhooks 或 Hawiyat API。每种方法支持各种平台，并提供简化的部署过程。
...
1. **启用自动部署**：在 Hawiyat 中的应用设置的常规标签中切换“自动部署”按钮。
...
- **分支匹配**：使用基于 Git 的提供商（GitHub、GitLab 等）时，确保 Hawiyat 中配置的分支与您打算推送到的分支匹配。不匹配将导致“分支不匹配”错误。
- 所有提供商的步骤相同。

## API 方法

通过 Hawiyat API 从任何地方以编程方式部署您的应用。

### 使用 API 部署的步骤

步骤：

1. **生成令牌**：在 Hawiyat 的个人资料设置中创建一个 API 令牌。
2. **获取 Compose ID**：
