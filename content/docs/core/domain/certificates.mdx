---
title: "Certificates"
description: "Learn how to set up certificates for the Hawiyat panel"
---

import { Callout } from 'fumadocs-ui/components/callout';

When using a domain for Hawiyat, we offer 3 options for certificates:
...
<Callout title="Cloudflare">When you register a domain on Cloudflare, the SSL certificate is automatically assigned by Cloudflare, so you need to select `None` in Hawiyat and disable HTTPS.</Callout>
...
1. In Hawiyat, select `Let's Encrypt` and enable `HTTPS`.
...
2. In Hawiyat, select `Let's Encrypt` and enable `HTTPS`.
