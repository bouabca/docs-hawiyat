...
description: "Learn how to set up and manage a cluster in Hawiyat with docker swarm."
sidebar:
  order: 1
---
...
A cluster is a group of nodes that work together to provide a single, unified service. In Hawiyat, clusters manage a group of servers collaborating to deliver this unified service.
...
- A server with Hawiyat installed.
...
4. Follow the two steps provided:
   ...
   - Install the Hawiyat agent. These steps are the same for both node types.
...
2. Fork the repository [Hawiyat/swarm-test](https://github.com/Hawiyat/swarm-test).
...
To add manager nodes to the cluster, follow the same steps as adding a worker node. However, if the manager node (where Hawiyat is running) fails, all other nodes will become inaccessible. This is because we currently have a single entry point for the cluster. To ensure high availability, you should have multiple manager nodes.
